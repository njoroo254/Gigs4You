...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _submit() async {
    ...
    // Kenyan phone: +254XXXXXXXXX or 07XXXXXXXX / 01XXXXXXXX
    final phoneRe = RegExp(r'^(\+254|0)[17]\d{8}$');
    if (!phoneRe.hasMatch(phone))
      return _err('Enter a valid Kenyan phone number (e.g. 0712345678 or +254712345678)');
    ...
    if (pass.length < 8) return _err('Password must be at least 8 characters');
    if (!RegExp(r'[0-9]').hasMatch(pass))
      return _err('Password must contain at least one number');
    ...
    if (result == null) return; // error shown via auth.error / snackbar below
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Min 8 characters, include a number',
