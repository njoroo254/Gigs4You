      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSt) => Padding(
          ...
          child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min,
            ...
            // Priority
            Text('Priority', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: ctx.appText3)),
                ...
                Expanded(child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: priority == p.$1 ? p.$3.withOpacity(0.1) : ctx.appSurfaceColor,
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(color: priority == p.$1 ? p.$3 : ctx.appBorderColor)),
                    child: Center(child: Text(p.$2,
                      style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700,
                        color: priority == p.$1 ? p.$3 : ctx.appText4)))))),
            ]),
            const SizedBox(height: 10),

            // Assign to agent
            Text('Assign to agent', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: ctx.appText3)),
            const SizedBox(height: 6),
  ...
  Widget _field(TextEditingController ctrl, String hint, IconData icon, {int maxLines = 1}) =>
    TextField(controller: ctrl, maxLines: maxLines,
      ...
      decoration: InputDecoration(
        hintText: hint, prefixIcon: Icon(icon, size: 18, color: context.appText4),
...
class _TaskList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (tasks.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(Icons.assignment_outlined, size: 56, color: context.appText4),
      const SizedBox(height: 12),
      Text('No tasks here', style: TextStyle(color: context.appText3, fontWeight: FontWeight.w600, fontSize: 15)),
    ]));
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (task.locationName != null && task.locationName!.isNotEmpty) ...[
        const SizedBox(height: 6),
        Row(children: [
          Icon(Icons.location_on_outlined, size: 12, color: context.appText4),
          const SizedBox(width: 3),
          Text(task.locationName!, style: TextStyle(fontSize: 12, color: context.appText4)),
        ]),
      ],
      if (task.dueAt != null) ...[
        const SizedBox(height: 4),
        Row(children: [
          Icon(Icons.schedule, size: 12, color: context.appText4),
          const SizedBox(width: 3),
          Text('Due ${_formatDate(task.dueAt!)}',
            style: TextStyle(fontSize: 12, color: task.isOverdue ? Colors.red : context.appText4)),
          if (task.isOverdue) ...[
            const SizedBox(width: 4),
            Container(
