...
class ManagerAgentsTab extends StatefulWidget {
  ...
  @override
  State<ManagerAgentsTab> createState() => _ManagerAgentsTabState();
}
...
class _ManagerAgentsTabState extends State<ManagerAgentsTab> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() => _agents =
          data.map((j) => Agent.fromJson(j as Map<String, dynamic>)).toList());
    } catch (_) {
      setState(() => _agents = []);
    }
  ...
  List<Agent> get _filtered {
    if (_filter == 'in_field')
      return _agents.where((a) => a.isCheckedIn).toList();
    if (_filter == 'offline')
      return _agents.where((a) => !a.isCheckedIn).toList();
    return _agents;
  }
  ...
  void _openChat(Agent agent) {
    final targetId =
        agent.user?.id.isNotEmpty == true ? agent.user!.id : agent.id;
    if (targetId.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Agent ID missing')));
      return;
    }

    Navigator.push(context,
        MaterialPageRoute(builder: (_) => ChatTab(initialOtherId: targetId)));
  }
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      _pill('${_agents.length - inField} offline',
                          AppColors.text4),
                    ]),
                  ]),
            ),
        ...
        if (_loading)
          ...
          SliverFillRemaining(
              child: Center(
                ...
                Text(
                    _filter == 'in_field'
                        ? 'No agents in the field right now'
                        : 'No agents found',
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
            ...
            Expanded(
                child: Column(
                  ...
                  Row(children: [
                    ...
                    _infoPill('${a.totalXp} XP', AppColors.text3),
                    const SizedBox(width: 6),
        ...
        Row(children: [
          Expanded(
              child: _statBox(
                  'Jobs done', '${a.completedJobs}', AppColors.primary)),
          const SizedBox(width: 10),
          Expanded(
              child: _statBox(
                  'Rating',
                  a.rating != null ? '⭐ ${a.rating!.toStringAsFixed(1)}' : '—',
                  AppColors.warning)),
          const SizedBox(width: 10),
          Expanded(
              child: _statBox('Status', a.isCheckedIn ? 'In field' : 'Offline',
                  a.isCheckedIn ? AppColors.primary : AppColors.text4)),
        ]),
        const SizedBox(height: 12),
            ...
            child: Row(children: [
              ...
              Expanded(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    const Text('Last known position',
                        style: TextStyle(
                            fontWeight: FontWeight.w700,
                            fontSize: 12,
                            color: AppColors.primary)),
                    Text(
                        '${a.lastLatitude!.toStringAsFixed(4)}, ${a.lastLongitude!.toStringAsFixed(4)}',
                        style: const TextStyle(
                            fontSize: 11,
                            color: AppColors.text3,
...
class _AgentCard extends StatelessWidget {
  ...
  const _AgentCard({required this.agent, required this.onTap, this.onMessage});
          ...
          child: Row(children: [
            Stack(children: [
              CircleAvatar(
                  radius: 22,
                  backgroundColor:
                      agent.isCheckedIn ? AppColors.primary : AppColors.surface,
