...
const _catColors = {
  ...
  'technician': Color(0xFFF97316),
  'logistics': Color(0xFF0D9488),
  'finance': Color(0xFF8B5CF6),
  'research': Color(0xFFEC4899),
  'merchandising': Color(0xFF1B6B3A),
...
class _JobsTabState extends State<JobsTab> {
                ...
                : provider.jobs.isEmpty
                    ...
                    : RefreshIndicator(
                        ...
                        child: ListView.separated(
                          ...
                          itemCount: provider.jobs
                              .length, // Removed AI recommendations for performance
                          ...
                          itemBuilder: (_, i) {
                            return JobCard(
                              job: provider.jobs[i],
                              onApply: () => _applyFlow(
                                  context, provider, provider.jobs[i]),
                            );
                          },
                        ),
                      ),
  ...
  Future<void> _applyFlow(
      BuildContext context, JobsProvider provider, Job job) async {
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
            ...
            children: [
              ...
              const Text('Cover note (optional)',
                  style: TextStyle(
                      fontSize: 12,
                      fontWeight: FontWeight.w600,
                      color: AppColors.text3)),
    ...
    if (confirmed == true && context.mounted) {
      final ok = await provider.applyForJob(job.id,
          coverNote: noteCtrl.text.isEmpty ? null : noteCtrl.text);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                ok ? '✅ Application submitted!' : 'Failed: ${provider.error}'),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(
          ...
          child:
            ...
            if (job.requiredSkills.isNotEmpty)
              Wrap(spacing: 5, runSpacing: 5, children: [
                ...
                if (job.requiredSkills.length > 4)
                  Text('+${job.requiredSkills.length - 4}',
                      style: const TextStyle(
            ...
            // Footer row
            Row(children: [
              const Icon(Icons.location_on, size: 13, color: AppColors.text4),
              const SizedBox(width: 3),
              Expanded(
                  child: Text(job.location,
                      style:
                          const TextStyle(fontSize: 11, color: AppColors.text4),
                      overflow: TextOverflow.ellipsis)),
